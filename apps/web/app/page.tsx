...
export default function Page(){
 ...
 useEffect(()=>{
  fetch(`${API}/api/v1/snapshot`).then(r=>r.json()).then(s=>{setSnap(s);setOps(s.opportunities||[]);setFills(s.fills||[]);let q:Record<string,Obj>={};(s.quotes||[]).forEach((x:Obj)=>q[`${x.venue}:${x.symbol}`]=x);setQuotes(q)}).catch(()=>{})
  ...
  ws.onmessage=m=>{const e=JSON.parse(m.data);const d=e.data||{}
   if(e.type==='snapshot'){setSnap(d);setOps(d.opportunities||[]);setFills(d.fills||[])}
 ...
 return <main>
  <header><div><div className="eyebrow"><Radio size={14}/> QUANT INTELLIGENCE TERMINAL</div><h1>Quant OS</h1></div>
   <div className="status"><span className={connected?'dot live':'dot'}/>{connected?'LIVE STREAM':'RECONNECTING'}<span className="pill">{(snap.mode||'...').toUpperCase()}</span><span className="pill safe">PAPER EXECUTION</span></div></header>
  ...
  <section className="grid lowerGrid">
   <Panel title="Paper Execution Log" right={`${fills.length} fills`}><table><thead><tr><th>Time</th><th>Side</th><th>Venue</th><th>Symbol</th><th>Qty</th><th>Price</th></tr></thead>
   <tbody>{fills.slice(0,10).map(f=><tr key={f.id}><td>{new Date(f.ts).toLocaleTimeString()}</td><td className={f.side==='buy'?'buy':'sell'}>{f.side.toUpperCase()}</td><td>{f.venue}</td><td>{f.symbol}</td><td>{num(f.quantity,6)}</td><td>{money(f.price)}</td></tr>)}</tbody></table></Panel>
   <Panel title="Strategy Lab" right="replay"><div className="lab"><h3>Cross-venue arbitrage</h3><p>Replay 5,000 synchronized quote batches using the same fee/slippage-aware detector.</p><button onClick={backtest}>Run backtest</button>
    {bt&&!bt.loading&&<div className="result"><Metric l="Net P&L" v={money(bt.net_pnl)}/><Metric l="Trades" v={String(bt.traded)}/><Metric l="Win rate" v={`${bt.win_rate}%`}/><Metric l="Costs" v={money(bt.estimated_costs)}/></div>}</div></Panel>
  </section>
  <section className="grid footerGrid">
   <Panel title="Risk Guardrails"><div className="guards"><Guard a="Minimum net edge" b="8 bp"/><Guard a="Max order notional" b="$2,500"/><Guard a="Max daily loss" b="$3,000"/><Guard a="Max drawdown" b="5%"/><Guard a="Max slippage" b="15 bp"/><Guard a="Execution" b="Paper"/></div></Panel>
...
function summary(e:any){let d=e.data||{};if(e.type==='quote')return `${d.venue} ${d.symbol} ${num(d.mid)}`;if(e.type==='opportunity')return `${d.symbol} +${num(d.net_edge_bps,1)}bp`;if(e.type==='fill')return `${d.side} ${d.symbol} @ ${num(d.price)}`;if(e.type==='risk_decision')return `${d.allowed?'allow':'reject'} ${d.reason}`;return ''}
