...
export default function Page(){
 ...
 const execMode=(snap.execution_mode||(snap.shadow_trading?'shadow':'paper')).toUpperCase()
 const execLabel=execMode==='SHADOW'?'SHADOW (NO ORDERS)':execMode==='LIVE'?'LIVE BLOCKED':'PAPER EXECUTION'
 ...
 return <main>
  ...
  <section className="kpis">
   ...
   <Card icon={<Zap/>} label="Opportunities" value={String(stats.opportunities||0)} sub={`${stats.trades||0} paper / ${stats.shadow_trades||0} shadow`}/>
  ...
  <section className="grid lowerGrid">
   <Panel title={execMode==='SHADOW'?'Shadow Decision Log':'Paper Execution Log'} right={`${(execMode==='SHADOW'?shadowFills:fills).length} rows`}><table><thead><tr><th>Time</th><th>Side</th><th>Venue</th><th>Symbol</th><th>Qty</th><th>Price</th></tr></thead>
